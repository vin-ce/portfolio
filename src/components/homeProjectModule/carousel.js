  ...
  if (!visualElList) {
    ...
    homeCarouselVisualsLinks.forEach((element, index) => {
      ...
      if (element.type === "image") {
        if (index === 0) {
          tempElList.push(
            <img key={`${projectID}_visual_${index}`} id={`${projectID}_visual_${index}`} className={image} src={element.src} onClick={nextVisual} draggable='false' />
          )
        } else {
          // hides other elements that aren't the first one
          tempElList.push(
            <img key={`${projectID}_visual_${index}`} id={`${projectID}_visual_${index}`} className={[image, hidden].join(" ")} src={element.src} onClick={nextVisual} draggable='false' />
          )
        }
        ...
        if (index === 0) {
          tempElList.push(
            <React.Fragment key={`${projectID}_visual_${index}`}>
              <VideoPlayer id={`${projectID}_visual_${index}`} src={element.src} onClick={nextVisual} isAutoplay={true} type="carousel" hasSound={element.hasSound} />
            </React.Fragment>
          )
        } else {
          // hides other elements that aren't the first one
          tempElList.push(
            <React.Fragment key={`${projectID}_visual_${index}`}>
              <VideoPlayer id={`${projectID}_visual_${index}`} propsClasses={[hidden].join(" ")} src={element.src} onClick={nextVisual} type="carousel" hasSound={element.hasSound} />
            </React.Fragment>
          )
        }
      }
  ...
  return (
    <div className={carouselContainer}>
      ...
      <div className={['leftCol', controlsContainer].join(' ')}>
        <span>
          <span>{slideIndex.current + 1}/{numOfVisuals}</span>
